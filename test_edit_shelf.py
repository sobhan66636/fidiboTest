...
def test_edit_shelf(driver):
    driver.get('https://fidibo.com/library/book/shelves')
    driver.maximize_window()
    wait = WebDriverWait(driver, 10)
    with allure.step('Create a new shelf with incorrect name'):
        ...
        name.send_keys(incorrect_name)
        confirm_shelf = driver.find_element(By.CLASS_NAME, 'add-shelve-btn')
        confirm_shelf.click()
        assert wait.until(EC.text_to_be_present_in_element((By.CLASS_NAME, "shelve-card-container"), incorrect_name)), "New item with incorrect name must be created"

    with allure.step('Edit the shelf name'):
        shelf = shelves_container.find_elements(By.CLASS_NAME, 'shelve-card-row')[1]
        driver.get(shelf.get_property('href'))
        edit = wait.until(EC.element_to_be_clickable((By.CLASS_NAME, 'shelve-page-buttons-edit')))
        edit.click();
        name = wait.until(EC.element_to_be_clickable((By.NAME, 'name')))
        time.sleep(2)
        name.clear()
        name.send_keys(correct_name)
        confirm_shelf = driver.find_element(By.CLASS_NAME, 'add-shelve-btn')
        confirm_shelf.click()
        wait.until(EC.staleness_of(confirm_shelf))
        driver.refresh()
        title = wait.until(EC.visibility_of_element_located((By.CLASS_NAME, 'shelve-page-title')))
        assert title.text == correct_name, "Title must be corrected"
    ...
    with allure.step('Delete the created shelf'):
        ...
        assert wait.until(EC.url_to_be('https://fidibo.com/library/book/shelves')), "Must redirect out when successfully deleted"
